...
6. Dispatch a fresh review agent only after the draft pull request exists and contains the commit under review. The review happens on that pull request so the reviewer has a destination for inline comments and its summary. A pre-PR review does not satisfy this workflow. The review agent itself publishes the review summary and every genuine actionable finding directly on the pull request. Do not ask it to return findings for the parent agent to publish. Review each cycle against the contract, full diff, tests, architecture, repository rules, review matrix, and maintainability guidance below. If there are no findings, the reviewer says so in the pull-request summary. Never invent or stretch a finding to produce a comment. A cycle is complete only after the reviewer returns the GitHub review URL or ID and the URL or ID of every inline comment. Treat a chat-only review result as incomplete, including after compaction, and verify the returned artifacts on the pull request before acting on the review.
7. Fix valid findings, reply with the change or a concrete disagreement, and resolve addressed threads. When a finding reveals a defect class, audit all structurally equivalent paths and update the review matrix before requesting re-review. Use focused checks while iterating, then run the required broad validation once the systemic audit is complete. Do not repeat unchanged broad checks merely to start another cycle. During agent review, request fresh re-review after material fixes, and repeat until there are no findings or all disagreements are recorded. Once agent review is clean, a user correction in task chat or a human PR comment becomes the active human review, including documentation-only feedback. Fix and respond without launching another review agent unless the human reviewer asks for one.
...
Review maintainability and cost as seriously as correctness. Trace changed behavior through its producers and consumers, then ask whether the change leaves redundant concepts, representations, conversions, traversals, policies, branches, allocations, or APIs. Examine module cohesion in both directions, including oversized files and one-function files created only to satisfy a size limit. Watch for duplicated behavior, speculative layers, hidden work in hot paths, unnecessary clones, and fallback or validation that repeats established phase contracts. These are prompts, not automatic findings. Check proposed replacements against compiler speed, generated code speed and size, and the issue scope. Report only problems supported by the code and a concrete improvement.
