...
Keep each language reference concise, evidence-backed, and useful on its own.
...
- Examples do not need to run on their own. They can assume stated imports and support declarations, and bodies can use `// ...` when behavior is irrelevant. Every shown form must still be syntactically and semantically valid, and non-`unit` bodies should retain the required result exit.
...
- Add a feature only after verifying the summary and example against authoritative repository sources.
- Update stale entries when the language changes instead of preserving compatibility guidance.
- Describe the current language, not the history of how it was implemented.
