...
**Remember:** [Block-shaped expressions](https://github.com/lejmer/bray/blob/develop/docs/language/expressions/sequenced-expressions.md) are self-delimiting, while other sequenced expressions require semicolons. A `yield` supplies the nearest value-producing region, a `return` exits the callable, and a `break` supplies a loop result. Use postfix `?` when an absent nullable should simply propagate; reserve nullable `match` for distinct state-specific control or transformation. A `try` unwraps one `Result` or `RunResult` layer. Evaluation otherwise follows source order.
