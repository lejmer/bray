...
`bytes<N>` is identical to `[u8; N]`. Local `let` bindings can write `bytes` when a byte string, element-list array,
integer-count repeated array, or already typed byte array supplies the extent. Other repeated counts need `bytes<N>`.
...
For `T?`, use `value.is_present()` or `value.is_absent()` when only the state Boolean is needed. Use postfix `?` to propagate absence, and use a nullable match only when the payload must be bound or the states need distinct control flow. State queries borrow, evaluate the receiver once, and work for non-copyable `T`.
...
| Represent optional presence           | [`T?`](https://github.com/lejmer/bray/blob/develop/docs/language/types/type-forms.md#nullable-type-form)                                                   | Use state queries for Boolean tests, `?` for propagation, and patterns only when binding or branching by state.              |
