# Patterns

Patterns structurally match values, refine control flow, introduce bindings, and determine how ownership or access paths flow into matched regions.

---

## Contents

- [Overview](patterns/overview.md)
- [Core pattern forms](patterns/core-pattern-forms.md)
- [Pattern resolution](patterns/pattern-resolution.md)
- [Binding patterns](patterns/binding-patterns.md)
- [Literal patterns](patterns/literal-patterns.md)
- [Union variant patterns](patterns/union-variant-patterns.md)
- [Product patterns](patterns/product-patterns.md)
- [Tuple and array patterns](patterns/tuple-and-array-patterns.md)
- [Nullable patterns](patterns/nullable-patterns.md)
- [Box patterns](patterns/box-patterns.md)
- [Alternative patterns](patterns/alternative-patterns.md)
- [Refutability](patterns/refutability.md)
- [Pattern contexts](patterns/pattern-contexts.md)
- [Pattern operation modes](patterns/pattern-operation-modes.md)
- [Partial moves through patterns](patterns/partial-moves-through-patterns.md)
- [Condition refinement](patterns/pattern-refinement.md)
- [Guards](patterns/guards.md)
- [Pattern evaluation](patterns/pattern-evaluation.md)
- [Summary](patterns/summary.md)
