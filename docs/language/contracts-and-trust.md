# Contracts and trust

Contracts and trust define caller obligations, postconditions, predicates, trusted capabilities, trusted obligations,
witness values, flow-sensitive contract reasoning, and trust boundaries.

---

## Contents

- [Overview](contracts-and-trust/overview.md)
- [Contract clauses](contracts-and-trust/contract-clauses.md)
- [Conditional execution guarantees](contracts-and-trust/execution-guarantees.md)
- [Ordinary requirements and postconditions](contracts-and-trust/preconditions-and-postconditions.md)
- [Trusted implementation capabilities](contracts-and-trust/trusted-implementation-capabilities.md)
- [Trusted caller obligations](contracts-and-trust/trusted-caller-obligations.md)
- [Predicates and predicate expressions](contracts-and-trust/predicates-and-predicate-expressions.md)
- [Static constraints](contracts-and-trust/static-constraints.md)
- [Predicate-expression callable calls](contracts-and-trust/predicate-expression-callable-calls.md)
- [Contract arithmetic](contracts-and-trust/contract-arithmetic.md)
- [Contract reasoning](contracts-and-trust/contract-reasoning.md)
- [Trusted witness values](contracts-and-trust/trusted-witness-values.md)
- [Trust boundaries](contracts-and-trust/trust-boundaries.md)
- [Obligation propagation](contracts-and-trust/obligation-propagation.md)
- [Lifecycle and foreign boundaries](contracts-and-trust/lifecycle-and-foreign-boundaries.md)
- [Summary](contracts-and-trust/summary.md)
