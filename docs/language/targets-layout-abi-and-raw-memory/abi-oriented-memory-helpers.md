...
ABI helpers can use `size_of<T>()`, `align_of<T>()`, `stride_of<T>()`, `layout_of<T>(count = count)`, `std.memory.offset`, and `std.memory.reinterpret` when their contracts preserve the required facts.
...
ABI helpers cannot manufacture initialized values by reinterpreting bytes unless their contract establishes the required `initialized_as<T>` or `initialized_range_as<T>` facts.
