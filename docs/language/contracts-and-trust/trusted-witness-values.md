...
- [recognized standard-library declarations](../compiler-known-and-standard-library/standard-library-recognition.md) whose language-defined contracts establish trusted guarantees.

A declaration introduces a witness value when its successful result carries trusted guarantees in `ensures(...)` and those conditions are tied to the returned value, its fields, its owned storage, its borrows, or its dependency contract.
...
If a trust boundary operand returns a witness value, the lasting conditions come from the returned value's contract, not from the trust boundary.
...
Storing a witness value stores its trusted guarantees only when the destination can preserve every dependency required by the value.
...
Copying a witness value is rejected unless the value's copy contract explicitly states how the copied value preserves or splits the trusted guarantees.

Observing or copying ordinary fields from a witness value does not copy the trusted guarantees carried by the witness value.

The trusted guarantees remain tied to the witness value and to the dependencies named by its contract.

Partially moving a witness value is rejected when the move would separate trusted guarantees from the value or dependency that carries them.

## Witness invalidation

The trusted guarantees carried by a witness value are invalidated when the witness value is moved from, consumed, destroyed, finalized, assigned `none`, replaced, or otherwise stops being live.

The trusted guarantees are also invalidated when a value, storage identity, allocation, borrow, scoped capability, resource, epoch, or version named by the carried conditions is invalidated.

Mutating a witness value, a referenced access path, or referenced storage invalidates every carried condition whose truth can depend on the mutated state.

Destroying or finalizing an owner invalidates witness conditions that depend on the owner's storage or resource state.

Leaving a `with` body invalidates witness conditions that depend on the scoped capability produced by that `with` expression unless the selected `exit` contract transfers the obligation to another live value.

Cancellation, panic propagation, `return`, `yield`, `break`, `continue`, nullable propagation, result propagation, and run-result propagation preserve witness conditions only along paths where the witness value and every dependency it names remain live and valid.

## Witness use in contracts

A trusted requirement can be satisfied by a live witness value when the condition carried by the witness value matches the required trusted predicate after ordinary path, field, equality, and dependency reasoning.

Guarantees carried by a witness value can satisfy matching trusted requirements at program points where the witness and its
...
A public API that exposes a witness value exposes the trusted guarantees and obligations carried by that value as part of its public contract.
