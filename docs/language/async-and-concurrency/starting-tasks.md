...
The receiver is evaluated before the empty argument list according to ordinary method-call order. `start()` consumes the owned
inactive computation, creates an independently running task, and returns its sole source-level `Task<T>` owner.

`start()` is synchronous because it performs only the transition into independently schedulable work. It does not wait for the task
