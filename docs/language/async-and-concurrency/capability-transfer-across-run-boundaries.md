...
A borrow or capability derived from thread-local static storage or other thread-affine state records the exact native-thread
attachment rather than only a compatible lane class. A task retaining that dependency is pinned to that exact thread and keeps the
attachment open until the dependency is resolved.
