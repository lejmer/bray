...
A single-yield region with result type other than `unit` must receive exactly one yielded value on every normal
completion path or have no normal continuation.
...
When `yield` targets a multi-yield region, it contributes an element and its continuation behavior is defined by that
multi-yield region's contract.
