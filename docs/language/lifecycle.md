# Lifecycle

Lifecycle rules define construction, finalization, destruction, scoped use, lifecycle ordering, and lifecycle obligations.

---

## Contents

- [Overview](lifecycle/overview.md)
- [Lifecycle declarations](lifecycle/lifecycle-declarations.md)
- [Lifecycle selection](lifecycle/lifecycle-selection.md)
- [Lifecycle ordering](lifecycle/lifecycle-ordering.md)
- [Construction](lifecycle/construction.md)
- [Finalization](lifecycle/finalization.md)
- [Destruction](lifecycle/destruction.md)
- [Scoped use](lifecycle/scoped-use.md)
- [With expressions](lifecycle/with-expressions.md)
- [Partial values and replacement](lifecycle/partial-values-and-replacement.md)
- [Scope exits, panics, and cancellation](lifecycle/scope-exits-panics-and-cancellation.md)
- [Lifecycle requirements in traits](lifecycle/lifecycle-requirements-in-traits.md)
- [API compatibility](lifecycle/api-compatibility.md)
- [Summary](lifecycle/summary.md)
