...
and checked two-phase cleanup plans. The backend must not lower every async call as a task or mandatory heap allocation. Direct await
has no task-control-block or scheduler semantics; `Async<T>.start()` is the independent task-storage boundary.
