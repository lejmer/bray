...
`platform.clock.monotonic_now` readings with one clock identity use one stable frequency and never decrease. `wall_now`
nanoseconds are below one billion. Public async entropy dispatches the blocking role to a checked blocking lane. Timer
