...
A compiler-known protected-representation type can have a language-defined copy contract only when the owning language model defines
that contract.
...
Copying a product copies each field according to field declaration order unless the product's owning model defines a narrower
