...
```text
cargo xtask standard-library os-bindings probe [--target <triple>]
```

The probe checks emitted constants, scalar mappings, layouts, field offsets, flexible tails, bitfield access, callable
signatures, linked symbols, thread-local storage, and dynamic lookup against the pinned SDK. A probe runs only on its
exact target host. Standard-library verification checks deterministic generated bytes and source-checks the generated
Bray declarations for every supported native target.
