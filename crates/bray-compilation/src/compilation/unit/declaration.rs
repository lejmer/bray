...
use bray_symbols::{AnySymbolId, PredicateDefinitionSymbolId, SymbolGraph, SymbolKey};
use bray_syntax::{
...
impl Compilation {
    ...
    pub(in crate::compilation) fn predicate_definition_key(
        ...
        self.declared_unit_key(owner.into_any(), BoundUnitKind::PredicateDefinition)
    }
...
mod tests {
    ...
    fn primary_declaration_lookups_share_one_inventory() {
        ...
        let callable_owner = symbols.symbol_for_key(callable.declared_owner()).unwrap();

        assert_eq!(
            compilation
                .declared_unit_key(callable_owner, BoundUnitKind::ConstantTemplate,)
                .unwrap(),
            None
        );
        ...
        assert_eq!(
            compilation
                .declared_unit_key(callable_owner, BoundUnitKind::ContractClause,)
                .unwrap(),
            None
        );
    ...
    #[test]
    fn concurrent_discovery_publishes_one_inventory() {
        let compilation = compilation(
            r"module app;
        ...
        std::thread::scope(|scope| {
            let requests = (0..4)
                .map(|_| {
                    ...
                    scope.spawn(move || {
                        ...
                        compilation.declared_units().unwrap()
                    })
                })
                .collect::<Vec<_>>();
            ...
            assert!(
                inventories
                    .iter()
                    .all(|index| std::ptr::eq(*index, compilation.declared_units().unwrap(),))
            );

            assert_eq!(compilation.declared_unit_keys().unwrap().len(), 2);
        });
    }
