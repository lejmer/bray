...
impl Compilation {
    ...
    fn compute_semantic_diagnostics(
        ...
        // Scheduled requests retain their shared unit identities beyond the inventory borrow.
        let roots = self
            .declared_unit_keys()?
            .iter()
            .cloned()
    ...
    > {
        ...
        let mut sources = vec![
            ...
            SemanticDiagnosticSource::CallableProofs(proofs),
...
enum SemanticDiagnosticSource {
    ...
    CallableProofs(Arc<PublishedUnitResult<Vec<bray_bound_tree::CallableProofObligation>>>),
...
impl SemanticDiagnosticSource {
    fn diagnostics(&self) -> &DiagnosticBag {
        match self {
            ...
            Self::CallableProofs(result) => result.result().diagnostics(),
