...
use bray_symbols::{
    ...
    GenericTypeParameterSymbolId, SemanticValueStore, TraitApplicationId, TypeData,
...
impl<'values> SemanticUnifier<'values> {
    ...
    pub(in crate::compilation::overlap) fn types_may_overlap(
        ...
        match (left_data.as_ref(), right_data.as_ref()) {
            (TypeData::Error, _) | (_, TypeData::Error) => false,
            (
                TypeData::Named {
                ...
                for (left, right) in left_arguments.iter().zip(right_arguments.iter()) {
                    match (left, right) {
                        ...
                        (
                            GenericArgumentTemplate::Type(left),
                            GenericArgumentTemplate::Type(right),
                        ) if !self.type_templates_may_overlap(left, right) => return false,
                        (
    ...
    fn header_type_parameter(&self, ty: TypeId) -> Option<GenericTypeParameterSymbolId> {
    ...
    fn arguments_may_overlap(&mut self, left: GenericArgument, right: GenericArgument) -> bool {
        match (left, right) {
    ...
    fn type_lists_may_overlap(&mut self, left: &[TypeId], right: &[TypeId]) -> bool {
        if left.len() != right.len() {
            return false;
        }
    ...
    fn constants_may_overlap(&mut self, left: ConstantTermId, right: ConstantTermId) -> bool {
        if left == right {
            return true;
        }
        ...
        match (left.as_ref(), right.as_ref()) {
            (ConstantTermData::Value(left), ConstantTermData::Value(right)) => left == right,
            (
