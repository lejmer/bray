        ...
        for (parameter, arguments, valid) in [
            ...
            let source = format!(
                "trusted module app; struct Policy {{ mut value: i32; }} \
                 ...
                 static func borrow(pos storage: &Self) -> &i32 executes(pure, total) {{ return &storage.value; }} \
                 static func borrow_mut(pos storage: &mut Self) -> &mut i32 executes(pure, total) {{ return &mut storage.value; }} \
