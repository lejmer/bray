...
use bray_symbols::{
    AnyConstantDefinitionId, AnySymbolId, ConstantDefinition, ConstantDefinitionFact,
    ConstantDefinitionState, ConstantInstanceKey, ConstantInstanceValueFact, ConstantTermData,
    ConstantTermId, ConstantValueData, ConstantValueKind, ErrorConstantDefinition, GenericOwnerId,
    GenericSubstitutionData, SemanticFactResult, SymbolFactRequest,
    ...
    > {
        ...
        let references = collect_references(bound, |_, target| match target {
            ...
            BoundReferenceTarget::Surface(symbol) => {
                ...
                let dependency = if definition == instance.definition() {
                    instance
                } else {
                    ConstantInstanceKey::new(
                        definition,
                        empty_concrete_substitution(values, definition)?,
                        selected_implementation_for_reference(definition, instance),
                    )
                };
...
fn substitute_expression_types(
    ...
    let substitution = instance.substitution().substitution();
    let entries = types
        .entries()
        .iter()
        .map(|entry| {
...
const fn selected_implementation_for_reference(
    definition: AnyConstantDefinitionId,
    owner: ConstantInstanceKey,
) -> Option<bray_symbols::ImplementationInstanceId> {
    match definition {
        AnyConstantDefinitionId::TraitMember(_) => owner.selected_implementation(),
...
mod tests {
    ...
    use bray_symbols::{
        AnyConstantDefinitionId, ConstantDefinitionState, ConstantInstanceKey, ConstantTermData,
    ...
    use super::empty_concrete_substitution;
    use crate::SelectedTarget;
    use crate::fact::{ConstantInstanceFactKey, FactCellTestEvent};
    ...
    fn concrete_substitution(
        ...
        let value = values
            .intern_constant_value(ConstantValueData::new(
                ty,
                ConstantValueKind::Integer(IntegerConstant::new(IntegerSign::NonNegative, [value])),
            ))
            .unwrap_or_else(|error| panic!("constant argument must be interned: {error:?}"));
