    ...
    C: CheckerRequestContext + ?Sized,
{
    ...
    fn infer_type(&mut self, expected: TypeId, actual: TypeId) -> bool {
        let expected_data = self.request.semantic_values().type_data(expected);
        ...
        if let TypeData::TypeParameter(parameter) = expected_data.as_ref() {
            ...
            if self.inferable.contains(&parameter) {
                return self.record(parameter, GenericArgument::Type(actual));
            }
        }
        ...
        let actual_data = self.request.semantic_values().type_data(actual);
        ...
        match (expected_data.as_ref(), actual_data.as_ref()) {
            ...
            (
                ...
                self.infer_type(*expected_element, *actual_element)
            ...
            (
                ...
                self.infer_type(*expected_storage, *actual_storage)
    ...
    fn infer_types(&mut self, expected: &[TypeId], actual: &[TypeId]) -> bool {
    ...
    fn infer_substitution(
        ...
        let expected = self
            .request
            .semantic_values()
            .generic_substitution_data(expected);

        let actual = self
            .request
            .semantic_values()
            .generic_substitution_data(actual);
    ...
    fn infer_constant(&mut self, expected: ConstantTermId, actual: ConstantTermId) -> bool {
        let expected_data = self.request.semantic_values().constant_term_data(expected);

        if let ConstantTermData::Parameter(parameter) = expected_data.as_ref() {
            let parameter = GenericParameterSymbolId::Const(*parameter);

            if self.inferable.contains(&parameter) {
                return self.record(parameter, GenericArgument::Constant(actual));
            }
        }
...
mod tests {
    ...
    fn nested_named_types_infer_open_type_arguments() {
        ...
        assert!(inference.infer_type(expected, actual));
