use bray_bound_tree::{
    ...
    BoundExpressionId, BoundOperator, BoundPatternId, BoundUnitView, CheckedBlockResult,
    CheckedBlockResultRole, CheckedControlFlowFactsBuilder, CheckedControlTransfer,
...
use crate::analysis::model::{
    AnalysisEdgeKind, AnalysisExitKind, AnalysisRefinement, AnalysisTaskOperationKind,
};

use super::model::{BlockTargetContext, ControlFlowGraphBuild, ControlFlowGraphBuilder};
#[cfg(test)]
use super::model::{ControlFlowGraphBuildOutcome, build_control_flow_graph};

impl<'view> ControlFlowGraphBuilder<'view> {
    pub(in crate::analysis) fn new(request: UnitCheckRequest<'view>) -> Self {
        Self {
            request,
            view: request.view(),
            storage: ControlFlowGraphAssembler::new(request.view().unit()),
            facts: CheckedControlFlowFactsBuilder::new(request.view().unit()),
            loops: Vec::new(),
            catches: Vec::new(),
            scopes: Vec::new(),
    ...
    pub(in crate::analysis) fn build_callable_body(
        &mut self,
        ...
        self.callable = Some(root);

        let completion = match body.kind() {
            bray_bound_tree::BoundCallableBodyKind::Block(body) => {
                self.build_block_with_role(body, block, CheckedBlockResultRole::CallableBody(root))
            }
            bray_bound_tree::BoundCallableBodyKind::Error(error) => match error.body() {
                Some(body) => {
                    self.build_block_with_role(body, block, CheckedBlockResultRole::Recovered)
                }
                None => {
                    self.push_exit(block, AnalysisExitKind::Recovery);
    ...
    pub(in crate::analysis) fn build_block(
        &mut self,
        id: BoundBlockId,
        current: AnalysisBlockId,
    ) -> Option<Option<AnalysisBlockId>> {
        self.build_block_with_role(id, current, CheckedBlockResultRole::Unit)
    }

    pub(in crate::analysis) fn build_block_with_role(
        &mut self,
        id: BoundBlockId,
        ...
        self.facts
            .push_block_result(CheckedBlockResult::new(id, role));

        self.scopes.push(id);
        self.block_targets.push(BlockTargetContext {
        ...
        let Some(target) = self.block_targets.pop() else {
            panic!("checker block-target stack lost the active lexical block");
        };

        if target.block != scope {
            panic!("checker scope and block-target stacks diverged");
        }
    ...
    pub(in crate::analysis) fn build_expression(
        &mut self,
        ...
        match expression {
            ...
            BoundExpression::ControlTransfer(expression) => {
                ...
                self.push_control_transfer(current, id, expression.kind(), expression.target());
    ...
    fn build_sequential_expression(
        ...
        for block in expression.child_blocks() {
            current = self
                .build_block_with_role(block, current, CheckedBlockResultRole::Expression(id))?
                .unwrap_or_else(|| self.push_block());
        }
    ...
    pub(in crate::analysis) fn build_expressions(
        &mut self,
    ...
    pub(in crate::analysis) fn build_operands(
        &mut self,
    ...
    pub(in crate::analysis) fn build_pattern(
        &mut self,
        id: BoundPatternId,
        current: AnalysisBlockId,
    ) -> Option<Option<AnalysisBlockId>> {
        ...
        self.record_pattern_fact(id, pattern);
        self.push_bound(current, id.into());
    ...
    fn push_control_transfer(
        ...
        let checked_target = match self.view.node_is_recovered(expression.into()) {
            Some(false) => self.checked_transfer_target(kind, target, target_loop),
            Some(true) | None => CheckedControlTransferTarget::Recovered,
        };

        self.facts
            .push_control_transfer(CheckedControlTransfer::new(expression, checked_target));
    ...
    pub(in crate::analysis) fn push_block(&mut self) -> AnalysisBlockId {
        self.storage.push_block()
    }

    pub(in crate::analysis) fn push_bound(&mut self, block: AnalysisBlockId, node: AnyBoundNodeId) {
    ...
    pub(in crate::analysis) fn push_recovery(
        &mut self,
        block: AnalysisBlockId,
        node: AnyBoundNodeId,
    ) {
    ...
    pub(in crate::analysis) fn push_direct_await(
        &mut self,
        block: AnalysisBlockId,
        expression: BoundExpressionId,
    ) {
    ...
    pub(in crate::analysis) fn push_task_operation(
        &mut self,
        block: AnalysisBlockId,
        expression: BoundExpressionId,
    ...
    pub(in crate::analysis) fn push_edge(
        &mut self,
    ...
    pub(in crate::analysis) fn push_exit(
        &mut self,
        block: AnalysisBlockId,
    ...
    pub(in crate::analysis) const fn scope_depth(&self) -> usize {
        self.scopes.len()
    }

    pub(in crate::analysis) const fn request(&self) -> UnitCheckRequest<'_> {
        self.request
    }

    pub(in crate::analysis) const fn view(&self) -> BoundUnitView<'_> {
        self.view
    }
    ...
    pub(in crate::analysis) fn finish(self, entry: AnalysisBlockId) -> ControlFlowGraphBuild {
        ControlFlowGraphBuild {
            graph: self.storage.finish(entry),
            facts: self.facts,
        }
    }
}
...
mod tests {
    use bray_bound_tree::{
        ...
        CheckedBlockResultRole, CheckedControlFlowFacts, CheckedControlTransferTarget,
        CheckedForIterationResolution, CheckedPatternResolution, MatchExhaustiveness, PatternTest,
    };
    ...
    fn lexical_exits_broadcast_task_cancellation_before_generic_lifecycle_resolution() {
        ...
        let tree = builder.finish();
        let graph = graph(&tree, &key, root);
        let facts = checked_facts(&tree, &key, root);
        ...
        assert_eq!(
            facts
                .control_transfer(return_expression)
                .map(|fact| fact.target()),
            Some(CheckedControlTransferTarget::Callable(root))
        );

        assert_eq!(
            facts.block_result(outer).map(|fact| fact.role()),
            Some(CheckedBlockResultRole::CallableBody(root))
        );
    }
    ...
    fn for_exhaustion_and_match_guard_failure_use_distinct_paths() {
        ...
        assert_eq!(
            facts
                .control_transfer(break_expression)
                .map(|fact| fact.target()),
            Some(CheckedControlTransferTarget::Iteration(for_expression))
        );

        assert_eq!(
            facts.block_result(for_body).map(|fact| fact.role()),
            Some(CheckedBlockResultRole::IterationBody(for_expression))
        );

        assert_eq!(
            facts.block_result(for_else).map(|fact| fact.role()),
            Some(CheckedBlockResultRole::Expression(for_expression))
        );

        assert!(matches!(
            facts
                .for_iteration(for_expression)
                .map(|fact| fact.resolution()),
            Some(CheckedForIterationResolution::Recovered)
        ));
        ...
        assert!(matches!(
            facts
                ...
                .map(|fact| fact.resolution()),
            Some(CheckedPatternResolution::Resolved {
                is_irrefutable: true,
        ...
        assert_eq!(
            facts
                .match_facts(match_expression)
                .map(|fact| fact.exhaustiveness()),
            Some(MatchExhaustiveness::Recovered)
        );
    }
    ...
    fn checked_facts(
        ...
        let request = request(tree, key, root);

        let crate::CheckerOutcome::Complete(result) = crate::analysis::check_control_flow(request)
        else {
            panic!("valid checked fact construction must complete");
        };

        result.into_parts().0.into_facts()
    }
    ...
    fn request<'tree>(
        tree: &'tree BoundTree,
        key: &'tree BoundUnitKey,
        root: bray_bound_tree::BoundCallableBodyId,
    ) -> UnitCheckRequest<'tree> {
        let Ok(request) = UnitCheckRequest::new(
            tree.view(key),
            UnitCheckRoot::CallableBody(root),
            semantic_values(),
