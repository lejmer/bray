use super::{CheckerInfrastructureError, CheckerQueryError, CheckerQueryResult};
...
pub trait CheckerRequestContext: Sync {
    ...
    /// Resolves the owned element of an authenticated standard-library raw buffer.
    fn raw_buffer_element(
        &self,
        _definition: NamedTypeSymbolId,
        _substitution: bray_symbols::GenericSubstitutionId,
    ) -> CheckerQueryResult<Option<TypeId>, Self::UpstreamError> {
        Ok(None)
    }
    ...
    /// Selects one lifecycle callable and its substituted signature.
    ...
    /// Checks source constant expressions embedded in one type template.
    fn checked_constant_terms(
        &self,
        template: &bray_symbols::TypeExpressionTemplate,
    ) -> CheckerQueryResult<DiagnosticResult<crate::CheckedConstantTerms>, Self::UpstreamError>
    {
        ...
        for occurrence in template.constant_expressions() {
            ...
            // The aggregate owns diagnostics after this dependency result drops.
