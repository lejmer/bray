...
fn canonical_relationships(
    ...
    for relationship in &relationships {
        ...
        if !relationship_kinds_are_valid(relationship.kind(), owner.kind(), member.kind()) {
...
fn relationship_kinds_are_valid(
    ...
    match relationship {
        SymbolRelationshipKind::PackageModule => {
            owner == SymbolKind::Package && member == SymbolKind::Module
        }
        SymbolRelationshipKind::ModuleMember => {
            owner == SymbolKind::Module && is_module_member(member)
        }
        ...
        SymbolRelationshipKind::TraitMember => {
            owner == SymbolKind::Trait && is_trait_member(member)
        }
        SymbolRelationshipKind::ImplementationMember => {
            owner == SymbolKind::InherentImplementation && is_type_or_implementation_member(member)
        }
        SymbolRelationshipKind::StructField => {
            owner == SymbolKind::Struct && member == SymbolKind::StructField
        }
        SymbolRelationshipKind::UnionVariant => {
            owner == SymbolKind::Union && member == SymbolKind::UnionVariant
        }
        ...
        SymbolRelationshipKind::GenericParameter => {
            supports_generic_parameters(owner)
                && matches!(
                    member,
        ...
        SymbolRelationshipKind::OverloadArm => matches!(
            owner,
        ...
        SymbolRelationshipKind::DefaultProvider => matches!(
            (owner, member),
            (
