...
use crate::{
    InterfacePredicateDefinition, InterfacePredicateDefinitionState, InterfaceSemanticFacts,
...
pub(super) fn validate_predicate_templates(
    ...
    for template in facts
        .declaration_templates
        .iter()
        .filter(|template| template.kind() == CheckedTemplateKind::PredicateDefinition)
    {
    ...
    for definition in &*facts.predicate_definitions {
        ...
        let expected =
            usize::from(definition.state() == InterfacePredicateDefinitionState::Defined);
