...
mod tests {
    ...
    fn text_inspection_renders_one_selected_unit_with_tree_guides() {
        ...
        let output = match render_bound_inspection(&compilation, target, OutputFormat::Text) {
            Ok(output) => output,
    ...
    fn inspection_outside_a_semantic_unit_is_explicit() {
        let compilation = compilation(SOURCE);
        let target = InspectionTarget::at(0, TextSize::ZERO);
    ...
    fn inspection_rejects_an_unknown_source() {
        let compilation = compilation(SOURCE);
        let target = InspectionTarget::at(1, TextSize::ZERO);
    ...
    fn inspection_rejects_an_offset_past_the_source() {
        ...
        let target = InspectionTarget::at(0, position);
    ...
    fn target_at(source: &str, needle: &str) -> InspectionTarget {
        let offset = source
            .find(needle)
    ...
    fn json_inspection(
        ...
        let output = match render_bound_inspection(compilation, target, OutputFormat::Json) {
            Ok(output) => output,
            Err(error) => panic!("bound inspection should render: {error:?}"),
        };
