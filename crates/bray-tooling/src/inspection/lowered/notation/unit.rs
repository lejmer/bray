...
pub(crate) fn render_unit(unit: &InspectionMirUnit, include_source: bool) -> String {
    ...
    for block in &unit.blocks {
        output.push('\n');
        push_block(&mut output, block, unit, include_source);
    }
...
fn push_block(
    ...
    if include_source {
        ...
        let _ = writeln!(output, "    bb{}{}{parameters}:", block.id, role);
    }

    for operation in &block.operations {
        super::operation::render(output, operation, unit, &block.source, include_source);
    }

    super::terminator::render(output, &block.terminator, &block.source, include_source);
}
