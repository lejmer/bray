...
impl<'project> ProjectCompiler<'project> {
    ...
    fn run_compiler(
        ...
        request.args(product.sources().iter().map(|source| {
            source.beneath(self.workspace_root).into_os_string()
        }));
        ...
        match self {
            Self::Check { interface } => {
                ...
                if let Some(interface) = interface {
                    request.arg("--emit-interface").arg(interface.into_os_string());
                }
            }
            Self::Build { output } => {
                request.arg("build").arg("--output").arg(output.into_os_string());
