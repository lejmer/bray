...
use crate::{DriverOutputFormat, run::exit_code_from_diagnostics};
...
impl TackCliError {
    ...
    pub const fn output_format(&self) -> DriverOutputFormat {
        match &self.kind {
            TackCliErrorKind::Clap(_) => DriverOutputFormat::Text,
...
impl Cli {
    fn into_invocation(self) -> Result<TackInvocation, TackCliError> {
        let output_format = DriverOutputFormat::from(self.output_format);
