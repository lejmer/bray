...
pub(crate) fn render_mir_inspection(
    ...
    let stdout = match output_format {
        DriverOutputFormat::Text => render_notation_text(&notations, &diagnostics, compilation),
...
mod tests {
    ...
    fn mir_notation_is_deterministic_and_code_like() {
        ...
        let first = render_mir_inspection(&compilation, target, DriverOutputFormat::Text)
            .unwrap_or_else(|error| panic!("MIR notation should render: {error:?}"));

        let second = render_mir_inspection(&compilation, target, DriverOutputFormat::Text)
