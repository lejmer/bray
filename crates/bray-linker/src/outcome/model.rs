use bray_diagnostics::{
    ...
    DiagnosticIoErrorKind, DiagnosticKind, DiagnosticLinkRequirement, DiagnosticNote,
...
fn link_failure_diagnostics_for_plan(plan: &LinkPlan, failure: &LinkFailure) -> DiagnosticBag {
    ...
    match failure {
        ...
        | LinkFailure::OptimizationReport(_) => {}
    }
...
fn link_failure_diagnostic(failure: &LinkFailure) -> Diagnostic {
    match failure {
        ...
        LinkFailure::OptimizationReport(problem) => optimization_report_diagnostic(problem),
    }
}
...
pub enum LinkFailure {
    ...
    OptimizationReport(crate::LinkOptimizationReportProblem),
}
...
pub struct LinkOutcome {
    ...
    optimization: Option<crate::LinkOptimizationReport>,
}
...
impl LinkOutcome {
    ...
    pub(crate) fn with_optimization_report(
        mut self,
        report: crate::LinkOptimizationReport,
    ) -> Self {
        if matches!(self.status, LinkStatus::Complete(_)) {
    ...
    /// Returns exact native optimization outcomes for a successful ThinLTO link.
    pub const fn optimization(&self) -> Option<&crate::LinkOptimizationReport> {
...
mod tests {
    ...
    fn every_terminal_link_failure_publishes_its_exact_diagnostic() {
        ...
        let cases = [
            ...
            (
                LinkFailure::OptimizationReport(
                    crate::LinkOptimizationReportProblem::Inaccessible(
