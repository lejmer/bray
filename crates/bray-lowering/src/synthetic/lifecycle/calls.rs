...
use bray_ir::{
    MirCall, MirCallTarget, MirCallableReference, MirFrameInitializer, MirOperand,
...
impl<C: SyntheticLoweringContext + ?Sized> SyntheticLowerer<'_, C> {
    ...
    pub(super) fn push_lifecycle_call(
        ...
        let completed = match execution {
            CallableExecution::Synchronous => {
                ...
                let value = operation.result().ok_or_else(|| {
    ...
    fn finalizer_call(
        ...
        let result = match execution {
            CallableExecution::Synchronous => bray_bound_tree::BoundCallResult::Immediate(result),
            CallableExecution::Asynchronous => {
                let future = self
                    .context
