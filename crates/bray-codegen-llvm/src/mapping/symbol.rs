...
pub(crate) fn declare_symbols<'context, 'mappings>(
    ...
    declare_static_storages(module, mappings, types)?;

    Ok(())
}

fn declare_static_storages<'context, 'mappings>(
    module: &Module<'context>,
    mappings: &'mappings CodegenMappings,
    types: &mut LlvmTypeMappings<'context, 'mappings>,
) -> Result<(), CodegenFailure> {
    for mapping in mappings.static_storages() {
        ...
        let ty = types.map(mapping.ty())?;
        let name = mapping.symbol().as_str();

        let global = module.get_global(name).unwrap_or_else(|| {
        ...
        declare_static_accessor(module, mappings, mapping, global.as_pointer_value(), types)?;
    }
...
fn declare_static_accessor<'context>(
    ...
    if module.get_function(&accessor_name).is_some() {
        return Ok(());
    }

    let pointer = types.default_pointer_type()?;

    let inkwell::types::BasicTypeEnum::PointerType(pointer) = pointer else {
    ...
    let comdat = module.get_or_insert_comdat(&accessor_name);

    accessor.as_global_value().set_comdat(comdat);

    let state_name = mapping.state_name();

    let state = module.get_global(&state_name).unwrap_or_else(|| {
        let state = module.add_global(types.context().i8_type(), None, &state_name);

        state.set_initializer(&types.context().i8_type().const_zero());
        state.set_linkage(Linkage::WeakODR);
        state.set_visibility(GlobalVisibility::Hidden);

        if mapping.instance().duration() == bray_symbols::StaticStorageDuration::ExactThread {
            state.set_thread_local(true);
        }

        let comdat = module.get_or_insert_comdat(&state_name);

        state.set_comdat(comdat);

        state
    });

    let initializer = mappings
        .instance_symbol(mapping.initializer())
        .ok_or(CodegenFailure::GeneratedModuleInvariant)?;

    let initializer_function = module
        .get_function(initializer.name().as_str())
        .ok_or(CodegenFailure::GeneratedModuleInvariant)?;

    if !initializer.signature().parameters().is_empty() {
        return Err(CodegenFailure::GeneratedModuleInvariant);
    }
    ...
    let initialize = context.append_basic_block(accessor, "static.initialize");
    let ready = context.append_basic_block(accessor, "static.ready");

    builder.position_at_end(entry);

    let current = builder
        .build_load(context.i8_type(), state.as_pointer_value(), "static.state")
    ...
    builder
        .build_conditional_branch(uninitialized, initialize, ready)
        .map_err(|_| CodegenFailure::GeneratedModuleInvariant)?;
    ...
    let value = invoke_static_initializer(
        &builder,
...
fn invoke_static_initializer<'context>(
    builder: &inkwell::builder::Builder<'context>,
    initializer: FunctionValue<'context>,
    ...
    let result_storage = match signature.result() {
        CodegenResultMapping::Indirect {
            ...
            let storage = builder
                .build_alloca(types.map(*pointee)?, "static.initializer.result")
                .map_err(|_| CodegenFailure::GeneratedModuleInvariant)?;
        ...
        CodegenResultMapping::Direct { .. } => None,
        CodegenResultMapping::Void => return Err(CodegenFailure::GeneratedModuleInvariant),
    };

    let call = builder
        .build_call(initializer, &arguments, "static.initializer")
        .map_err(|_| CodegenFailure::GeneratedModuleInvariant)?;

    call.set_call_convention(initializer.get_call_conventions());
    ...
    match signature.result() {
        ...
        CodegenResultMapping::Indirect { .. } => {
            let (storage, pointee) =
                result_storage.ok_or(CodegenFailure::GeneratedModuleInvariant)?;

            builder
                .build_load(types.map(pointee)?, storage, "static.initializer.value")
                .map_err(|_| CodegenFailure::GeneratedModuleInvariant)
        }
