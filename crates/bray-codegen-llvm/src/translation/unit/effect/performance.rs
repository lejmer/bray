...
pub(super) struct PerformanceLoop<'context> {
    ...
    iteration: PhiValue<'context>,
    inner_iterations: std::num::NonZeroU64,
}

impl<'context, 'module, 'request, 'types> UnitTranslator<'context, 'module, 'request, 'types> {
    pub(super) fn begin_performance_interval(
        &self,
        function: FunctionValue<'context>,
    ) -> Result<Option<PerformanceLoop<'context>>, CodegenFailure> {
        ...
        let header = self
            .types
            .context()
            .append_basic_block(function, "performance.iteration");

        let body = self
            .types
            .context()
            .append_basic_block(function, "performance.execute");
    ...
    pub(super) fn finish_performance_interval_iteration(
        &self,
        performance_loop: Option<PerformanceLoop<'context>>,
        successful: IntValue<'context>,
        ...
        retain_performance_value(self.types.context(), &self.builder, successful)?;

        let Some(performance_loop) = performance_loop else {
            self.observe_performance_interval(
        ...
        let within_count = llvm(
            self.builder.build_int_compare(
                ...
                "performance.iteration.within_count",
            ),
        )?;

        let more = llvm(self.builder.build_and(
            successful,
            within_count,
        ...
        let done = self
            .types
            .context()
            .append_basic_block(performance_loop.function, "performance.iteration.done");
