use super::query::{format_checker_query, format_checker_symbol_kind};

pub(in crate::catalog::english::argument::native) fn format_english_checker_failure(
    ...
    let (is_internal, message) = match failure {
        ...
        Failure::SemanticQueryUnavailable { symbol, query } => {
            return super::super::format_internal_compiler_error(format!(
        ...
        Failure::GenericSubstitution(failure) => {
            return super::super::format_internal_compiler_error(
                format_generic_substitution_failure(failure),
            );
        }
        Failure::SemanticValue(failure) => {
            return super::super::artifact::format_english_semantic_value_failure(failure);
        }
        ...
        Failure::InvalidExpressionTypeInput { expression } => {
            ...
            return super::super::format_internal_compiler_error("associated the highlighted expression with the wrong source body while determining its type".to_string());
        }
        ...
        Failure::LiteralValue(failure) => {
            return super::super::format_internal_compiler_error(format_literal_value_failure(
                failure,
            ));
        }
        Failure::PatternInput(failure) => {
            return super::super::format_internal_compiler_error(format_pattern_input_failure(
                failure,
            ));
        }
        Failure::ConstantInput(failure) => {
            return super::super::format_internal_compiler_error(format_constant_input_failure(
                failure,
            ));
        }
        Failure::ConstantEvaluation(failure) => {
            return super::super::format_internal_compiler_error(
                format_constant_evaluation_failure(failure),
            );
        }
        Failure::ConstantOperation(failure) => {
            return super::super::format_internal_compiler_error(
                format_constant_operation_failure(failure),
            );
        }
        ...
        Failure::SemanticSelection(failure) => {
            return super::super::format_internal_compiler_error(
                format_semantic_selection_failure(failure),
            );
        }
        ...
        Failure::StoragePlan(failure) => {
            return super::super::format_internal_compiler_error(format_storage_plan_failure(
                failure,
            ));
        }
        ...
        Failure::StorageFlow(failure) => {
            return super::super::format_internal_compiler_error(format_storage_flow_failure(
                failure,
            ));
        }
    ...
    if is_internal {
        super::super::format_internal_compiler_error(message)
    } else {
        message.to_owned()
    }
}
...
fn format_contextual_checker_failure(
    ...
    Some(super::super::format_internal_compiler_error(detail))
}

pub(in crate::catalog::english::argument::native) fn format_generic_substitution_failure(
