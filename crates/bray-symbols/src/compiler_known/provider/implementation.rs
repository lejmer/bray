...
use super::validation::{validate_declaration_id, validate_scope_id};
...
impl CompilerKnownSymbolProvider {
    ...
    fn build_from_catalog(
        ...
        let (declaration_symbols, descriptor_symbols) =
            allocate_declarations(catalog, &mut allocator)?;
...
fn allocate_declarations(
    ...
    for (index, descriptor) in catalog.compiler_known_declarations().iter().enumerate() {
        ...
        let owner_kind = declaration_owner_kind(catalog, descriptor)?;
        let kind = declaration_symbol_kind(descriptor.kind().into(), owner_kind);
...
fn declaration_owner_kind(
    catalog: &CompilerKnownCatalog,
    descriptor: &CompilerKnownDeclarationDescriptor,
) -> Result<SymbolKind, CompilerKnownSymbolBuildError> {
    match descriptor.owner() {
        CompilerKnownDeclarationOwner::Scope(_) => Ok(SymbolKind::CompilerKnownEnvironment),
        CompilerKnownDeclarationOwner::Declaration(owner) => {
            let Some(owner) = catalog.compiler_known_declaration(owner) else {
