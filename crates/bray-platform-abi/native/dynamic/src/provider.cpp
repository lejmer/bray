...
bool ranges_overlap(
    ...
    if (left_length > std::numeric_limits<std::uintptr_t>::max() - left_start ||
...
BrayPlatformStatus load_path(BrayPlatformPath path, std::uint32_t policy, NativeLibrary* library) {
    if (path.address == nullptr || path.length == 0 || path.length % sizeof(wchar_t) != 0) {
    ...
    if (native_path.find(L'\0') != std::wstring::npos || !absolute_path(native_path)) {
        return status(invalid_input);
    }
...
BrayPlatformStatus load_path(BrayPlatformPath path, std::uint32_t policy, NativeLibrary* library) {
    if (path.address == nullptr || path.length == 0 ||
    ...
    if (native_path.find('\0') != std::string::npos || !absolute_path(native_path)) {
        return status(invalid_input);
    }
...
extern "C" BrayPlatformStatus bray_platform_dynamic_library_open_path(
    ...
    if (opened == nullptr || !aligned(opened, alignof(std::uint64_t)) ||
        (path.address != nullptr && ranges_overlap(path.address, path.length, opened, sizeof(*opened)))) {
        return status(invalid_input);
    }
...
extern "C" BrayPlatformStatus bray_platform_dynamic_library_symbol(
    ...
    if (name == nullptr || length == 0 ||
        length > std::numeric_limits<std::size_t>::max() || address == nullptr ||
