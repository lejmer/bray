    ...
    while group < groups.len() {
        ...
        while index < groups[group].len() {
            ...
            while previous_group <= group {
                let limit = if previous_group == group { index } else { groups[previous_group].len() };
                let mut previous = 0;

                while previous < limit {
                    let (other_name, other_symbol) = groups[previous_group][previous];

                    assert!(!equal(name, other_name), "duplicate role identity in runtime catalog");

                    if let (Some(symbol), Some(other)) = (symbol, other_symbol) {
                        assert!(!equal(symbol, other), "duplicate native symbol in runtime catalog");
                    }
