...
fn main() {
    ...
    let target = NativeTarget::ALL.into_iter().find(|target| target.as_str() == triple)
    ...
    let archive = TargetOutputName::for_native(target.object_format(), TargetOutputKind::StaticLibrary)
        .file_name("bray_runtime_bootstrap").expect("the bootstrap archive has a valid name");

    println!("cargo:rerun-if-changed={}", root.join("runtime").display());
    println!("cargo:rerun-if-changed={}", root.join("standard-library").display());
